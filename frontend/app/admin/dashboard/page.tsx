...
export default function AdminDashboardPage() {
  return <AdminRouteFrame title="ダッシュボード" description="求人通知作成の入口と、現在利用できる機能を確認します。">{(mode) => <div className="admin-route-grid">
    ...
    <section className="admin-card"><h3>現在の送信モード</h3><p className="admin-status">{modePresentation(mode).label}</p><p className="admin-muted">累計件数や全operationの集計は、取得手段がないため表示していません。</p></section>
  </div>}</AdminRouteFrame>;
}
