...
export default function AdminPage() {
  ...
  const validationInput = useMemo<ValidationInput>(() => ({
    ...
    message,
    notificationLink: isLiveMode(lineSendMode) ? notificationLink : job?.job_url ?? null,
  ...
  const workflowState = deriveNotificationUiState({
    ...
    liveBlocked: !canAttemptSend(lineSendMode, validation?.sendable_count ?? 0) || (isLiveMode(lineSendMode) && !notificationLink),
    liveLinkReady: !isLiveMode(lineSendMode) || Boolean(notificationLink),
  ...
  useEffect(() => {
    if (!isLiveMode(lineSendMode) || !canAttemptSend(lineSendMode) || !selectedJobId) {
